...
function ErrorPrompt(Message,state)
    if getrenv then
        ...
        prompt:setErrorTitle("SimpleSpy V3 出错了")
...
local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 0),Size = UDim2.new(0, 57, 0, 18),Font = Enum.Font.SourceSansBold,Text =  "远程监听",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
...
local TextLabel = Create("TextLabel",{Parent = ToolTip,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,Position = UDim2.new(0, 2, 0, 2),Size = UDim2.new(0, 196, 0, 46),ZIndex = 3,Font = Enum.Font.SourceSans,Text = "悬停在按钮上会显示说明。",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextWrapped = true,TextXAlignment = Enum.TextXAlignment.Left,TextYAlignment = Enum.TextYAlignment.Top})
...
function newButton(name, description, onClick)
    ...
    Button.MouseButton1Click:Connect(function(...)
        ...
        if ToolTip.Visible and type(description) == "function" then
            local ok, tip = pcall(description)
            if ok and tip then
                TextLabel.Text = tip
                TextLabel.TextScaled = true
            end
...
function newRemote(type, data)
    ...
    local log = {
        Name = remote.name,
        Function = data.infofunc or "-- 函数信息未启用",
        Remote = remote,
        DebugId = data.id,
    ...
    local connect = Button.MouseButton1Click:Connect(function()
        ...
        if blocked then
            log.GenScript = "-- SimpleSpy 已拦截这个远程调用，未发送到服务器\n\n" .. log.GenScript
        end
...
if not getgenv().SimpleSpyExecuted then
    local succeeded,err = pcall(function()
        if not RunService:IsClient() then
            error("SimpleSpy 不能在服务器端运行！")
        end
        getgenv().SimpleSpyShutdown = shutdown
        onToggleButtonClick()
        if not hookmetamethod then
            ErrorPrompt("你的执行器不支持 hookmetamethod，SimpleSpy V3 的部分功能会不可用。",true)
        end
...
newButton(
    "复制代码",
    function() return "复制右侧代码框里的内容" end,
    function()
...
newButton(
    "复制路径",
    function() return "复制当前远程对象的路径" end,
    function()
...
newButton("执行代码",
    function() return "重新执行这次远程调用" end,
    function()
...
newButton(
    "复制脚本",
    function() return "复制触发该远程调用的脚本路径\n提示：这个功能不太稳定，nil 表示没找到" end,
    function()
        if selected then
...
newButton("函数详情",function() return "查看触发函数的详细信息" end,
function()
    ...
    if func then
        ...
        if typeoffunc ~= 'string' then
            ...
            codebox:setRaw("--[[正在把表格转换成代码，请稍等]]")
            selected.Function = v2v({functionInfo = info})
        end
        codebox:setRaw("-- 调用函数信息\n-- 由 SimpleSpy V3 序列化器生成\n\n"..selected.Function)
        TextLabel.Text = "完成！函数信息已生成。"
    else
        TextLabel.Text = "错误：没有找到选中的函数。"
    end
end)
...
newButton(
    "排除此项",
    function() return "让 SimpleSpy 忽略当前远程对象。\n只是从日志里排除，不会影响它正常使用。" end,
    function()
        if selected then
...
newButton(
    "排除同名",
    function() return "让 SimpleSpy 忽略所有同名远程对象。\n只是从日志里排除，不会影响它们正常使用。" end,
    function()
        if selected then
...
--- clears blacklist
newButton("清排除表",
function() return "清空排除列表。\n清空后，这些远程对象会重新出现在日志里。" end,
function()
    blacklist = {}
...
newButton(
    "拦截此项",
    function() return "拦截当前远程对象，阻止它发送到服务器。\n它仍会出现在日志里，只是不再真正发出去。" end,
    function()
        if selected then
...
newButton("拦截同名",function()
    return "拦截所有同名远程对象，阻止它们发送到服务器。\n它们仍会出现在日志里，只是不再真正发出去。" end,
    function()
        if selected then
...
newButton(configs.logcheckcaller and "记录本机：开" or "记录本机：关",function()
    return ("当前：%s｜记录客户端自己触发的远程调用"):format(configs.logcheckcaller and "开启" or "关闭")
end,
function(FunctionTemplate)
    configs.logcheckcaller = not configs.logcheckcaller
    local label = FunctionTemplate:FindFirstChild("Text")
    if label then
        label.Text = configs.logcheckcaller and "记录本机：开" or "记录本机：关"
    end
    TextLabel.Text = ("当前：%s｜记录客户端自己触发的远程调用"):format(configs.logcheckcaller and "开启" or "关闭")
end)
...
newButton(configs.advancedinfo and "高级信息：开" or "高级信息：关",function()
    return ("当前：%s｜显示更多远程调用信息"):format(configs.advancedinfo and "开启" or "关闭")
end,
function(FunctionTemplate)
    configs.advancedinfo = not configs.advancedinfo
    local label = FunctionTemplate:FindFirstChild("Text")
    if label then
        label.Text = configs.advancedinfo and "高级信息：开" or "高级信息：关"
    end
    TextLabel.Text = ("当前：%s｜显示更多远程调用信息"):format(configs.advancedinfo and "开启" or "关闭")
end)

newButton("复制DC邀请",function()
    return "复制 SimpleSpy 的 Discord 邀请链接。部分执行器不会自动打开 Discord，只能复制链接。"
end,
function()
    local ok = pcall(setclipboard, "https://discord.com/invite/AWS6ez9")
    TextLabel.Text = ok and "已复制邀请链接，打不开就手动粘贴" or "复制失败：当前执行器不支持剪贴板"
    if request then
        pcall(function()
...
if configs.supersecretdevtoggle then
    newButton("加载SSV2.2",function()
        return "加载 SimpleSpy V2.2"
    end,
    function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/exxtremestuffs/SimpleSpySource/master/SimpleSpy.lua"))()
    end)
    newButton("加载SSV3",function()
        return "加载 SimpleSpy V3"
    end,
    function()
