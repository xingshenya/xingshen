...
local giveup = tick() + 15
repeat task.wait() until Confirmed or tick() > giveup
...
TeleSection:Input({
    Title = "目标坐标", Desc = "格式：X,Y,Z", Default = "0,0,0",
    Callback = function(text) inputCoord = text end
})
        ...
        if state then
            shotbatKillThread = task.spawn(function()
                while shotbatKillEnabled do
                    ...
                    local char = player.Character
                    if char then
                        local tool = char:FindFirstChild("Shotbat")
                        if not tool then tool = player.Backpack:FindFirstChild("Shotbat") end
                        if tool then
        ...
        if state then
            tripbatKillThread = task.spawn(function()
                while tripbatKillEnabled do
                    ...
                    local char = player.Character
                    if char then
                        local tool = char:FindFirstChild("Subspace Tripbat")
                        if not tool then tool = player.Backpack:FindFirstChild("Subspace Tripbat") end
                        if tool then
        ...
        if state then
            gubbyThread = task.spawn(function()
                while gubbyEnabled do
                    ...
                    local char = player.Character
                    if char then
                        local tool = char:FindFirstChild("Gubby Bat")
                        if not tool then tool = player.Backpack:FindFirstChild("Gubby Bat") end
                        if tool then
                            local humanoid = char:FindFirstChildOfClass("Humanoid")
        ...
        if state then
            poisonKillThread = task.spawn(function()
                while poisonKillEnabled do
                    ...
                    local char = player.Character
                    if char then
                        local tool = char:FindFirstChild("Poison Bat")
                        if not tool then tool = player.Backpack:FindFirstChild("Poison Bat") end
                        if tool then
...
toggleRefs.attract = Tabs.WeirdBatTab:Toggle({
    Title = "吸人", Desc = "自动传送到最近的存活玩家，无目标时回到安全点",
    Value = false,
    Callback = function(state)
                        ...
                        for _, p in ipairs(players) do
                            if p ~= player then
                                ...
                                if char and char:FindFirstChild("HumanoidRootPart") then
                                    ...
                                    if humanoid and humanoid.Health > 0 and not hasForceField then
        ...
        if state then
            aquaKillThread = task.spawn(function()
                while aquaKillEnabled do
                    ...
                    local char = player.Character
                    if char then
                        local tool = char:FindFirstChild("Aqua Bat")
                        if not tool then tool = player.Backpack:FindFirstChild("Aqua Bat") end
                        if tool then
...
toggleRefs.electroKill = Tabs.WeirdBatTab:Toggle({
    Title = "五雷轰顶(咖喱棒)",
    Desc = "疯狂释放雷电技能（自动瞄准敌人，无目标时保持上一位置）",
    Value = false,
    Callback = function(state)
            ...
            electroKillThread = task.spawn(function()
                ...
                local function getDefaultTarget()
                    ...
                    if char and char:FindFirstChild("HumanoidRootPart") then
                        ...
                        return root.Position + root.CFrame.LookVector * 10
                    end
                ...
                while electroKillEnabled do
                    ...
                    local char = player.Character
                    if char then
                        local tool = char:FindFirstChild("Electro Bat")
                        if not tool then tool = player.Backpack:FindFirstChild("Electro Bat") end
                        if tool then
                            local root = char:FindFirstChild("HumanoidRootPart")
                ...
                while antiFallEnabled do
                    ...
                    if char and char:FindFirstChild("HumanoidRootPart") then
                        ...
                        if root.Position.Y < 5 then
                            root.CFrame = CFrame.new(78, 179, -3)
                        end
                    end
                    task.wait(0.1)
                end
    ...
    Callback = function()
        task.spawn(function()
            for i = 1, 100 do
                ...
                local char = player.Character
                if char then
                    local tool = char:FindFirstChild("Power Bat")
                    if not tool then tool = player.Backpack:FindFirstChild("Power Bat") end
                    if tool then
