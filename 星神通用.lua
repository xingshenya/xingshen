...
-- 从工具中提取攻击配置（若没有则使用默认模板）
local function buildAttackData(tool)
    local defaultData = {
        attackCycleData = {
            knockbackMul = 1,
            slowMult = 0.2,
        ...
        if s then
            autoAttackThread = task.spawn(function()
                ...
                while autoAttackEnabled do
                    ...
                    -- 构建攻击数据（含工具引用）
                    local attackData = buildAttackData(tool)
                    ...
                    for _, player in ipairs(Players:GetPlayers()) do
                        ...
                        -- 发送攻击请求
                        pcall(function()
                            remote:InvokeServer("AttemptWeaponHit", attackData, {hitData})
                        end)

                        task.wait(0.01)  -- 间隔0.01秒
                    end

                    task.wait()  -- 一轮结束后再继续循环（避免空转）
                end
            end)
        end
    end
